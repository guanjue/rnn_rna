def prepare_bed2matrix(input_all,input_celltype_ideas,output):
	...
	for records in data_all1:
		if records[0] in data_ideas1:
			...
			result.write(tmp[0]+'\t'+tmp[1]+'\t'+tmp[2]+'\t'+tmp[5]+'\t'+'1000'+'\t'+'.'+'\t'+tmp[1]+'\t'+tmp[2]+'\t'+tmp[6]+'\t'+tmp[3]+'\n')
		else:
			tmp=records
			result.write(tmp[2]+'\t'+tmp[3]+'\t'+tmp[4]+'\t'+'NA'+'\t'+'1000'+'\t'+'.'+'\t'+tmp[3]+'\t'+tmp[4]+'\t'+'255,255,255'+'\t'+tmp[0]+'\n')
	result.close()
	data_all0.close()
	data_ideas0.close()
...
############################################################################
### python prepare_bed2matrix.py -a data_matrix_binarypattern_sorted.txt -i used_DNA_intervals.1.bed -o used_DNA_intervals.1.colored.bed
import getopt
import sys
def main(argv):
	try:
		...
		print 'python prepare_bed2matrix.py -a data_matrix_binarypattern_sorted.txt -i used_DNA_intervals.1.bed -o used_DNA_intervals.1.forbed2matrix.bed'
		sys.exit(2)
	...
	prepare_bed2matrix(input_all,input_celltype_ideas,output)

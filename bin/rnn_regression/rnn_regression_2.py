...
x = tf.placeholder(tf.float32, shape=[None, sec_d_genebody, thr_d, for_d])
...
h_conv1_out = tf.split(x_reshape,axis=0, num_or_size_splits=sec_d_genebody)
...
y_conv = (tf.matmul(lstm_tail, W_atten1)) + (tf.matmul(lstm_tail_2, W_atten1_2)) 
...
for i in range(iter_num):
	...
	batch_xs1=np.concatenate((x_train[batch_id,0:sec_d,:,:], x_train[batch_id,sec_d:sec_d_genebody,:,:][:,::-1,:,:]), axis=1)
	...
	#print(batch_ys1.shape)
	sess.run(train_step, feed_dict={x: batch_xs1, x_2: batch_xs1_2, y_: batch_ys1, keep_prob1: 0.5})
	...
	if i%100 == 0:
		...
		y_p = sess.run(y_conv, feed_dict={x: batch_xs1, x_2: batch_xs1_2, keep_prob1: 1})
		y_obs=batch_ys1
	...
	if i%1000 == 0:
		y_p = sess.run(y_conv, feed_dict={x: np.concatenate((x_test[:,0:sec_d,:,:], x_test[:,sec_d:sec_d_genebody,:,:][:,::-1,:,:]), axis=1), x_2: x_test[:,:sec_d_rev:-1,:,:], keep_prob1: 1})
		y_obs=y_test[:,:]
		y_mean=np.mean(y_obs)
		print(y_mean)
		...
		if 1==1:#accuracy_r2_test<=r2_test and accuracy_r2_train<=r2_train:
			saver = tf.train.Saver()
			save_path = saver.save(sess, "trained_rnn_2.ckpt")

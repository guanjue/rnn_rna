...
print(data_ideas_matrix.shape)
### read rna-seq
data_rna, data_rna_sig = read_rna2matrix('/Users/gzx103/Documents/zhang_lab/projects/atacseq_vision/rna_seq_pred/rsem/lsk_bm.rsem.txt',1,4)
...
for ct in cell_types:
	...
	y_test_sig = np.concatenate((y_test_sig, data_rna_sig[pos_test_id,:]), axis = 0)
	y_train_sig = np.concatenate((y_train_sig, data_rna_sig[pos_train_id,:]), axis = 0)
	#x_all = np.concatenate((x_all, data_ideas_matrix), axis = 0)
	#y_all = np.concatenate((y_all, data_rna), axis = 0)
	#y_all_sig = np.concatenate((y_all_sig, data_rna_sig), axis = 0)
...
saver.restore(sess, "trained_rnn_3_noatac_binary.ckpt")
...
for ct in cell_types:
	...
	y_conv_pred_nosoftmax_test = sess.run(y_conv, feed_dict={x_1: x_test[:,0:sec_d,:,:], x_2: x_test[:,:sec_d_rev:-1,:,:], x_3: x_test[:,sec_d:sec_d_genebody,:,:], y_: y_test, keep_prob1: 1.0})
	...
	write2d_array(y_conv_pred_test,'noatac3/y_conv_pred_all.'+ct+'.txt')
	write2d_array(y_conv_obs_test,'noatac3/y_conv_obs_all.'+ct+'.txt')
	write2d_array(y_test_sig,'noatac3/data_rna_sig.'+ct+'.txt')
	#write2d_array(y_conv_pred_nosoftmax_test,'noatac3/y_conv_pred_nosoftmax_test.'+ct+'.txt')
	...
	#rnn_hidden_layer_pred_dif[rnn_hidden_layer_pred_dif<=-6] = -6
	#write2d_array(rnn_hidden_layer_pred_1,'noatac3/rnn_hidden_layer_pred_1_nosoftmax.'+ct+'.txt')
	write2d_array(rnn_hidden_layer_pred_softmax_1,'noatac3/rnn_hidden_layer_pred_1.'+ct+'.txt')
	write2d_array(rnn_hidden_layer_pred_softmax_2,'noatac3/rnn_hidden_layer_pred_2.'+ct+'.txt')
	write2d_array(rnn_hidden_layer_pred_softmax_3,'noatac3/rnn_hidden_layer_pred_3.'+ct+'.txt')

	write2d_array(y_test_sig,'noatac3/y_test_sig.'+ct+'.txt')
	write2d_array(y_test,'noatac3/y_test.'+ct+'.txt')

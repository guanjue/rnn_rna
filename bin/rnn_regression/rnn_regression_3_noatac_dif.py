...
### read ideas labels
data_ideas_matrix = read_tab2matrix('/Users/gzx103/Documents/zhang_lab/projects/atacseq_vision/rna_seq_pred/matrix_for_rnn/gene_list.lsk_bm.0.tab',1,6)
...
for i in range(1,17):
	print(i)
	data_ideas_tmp = read_tab2matrix('/Users/gzx103/Documents/zhang_lab/projects/atacseq_vision/rna_seq_pred/matrix_for_rnn/gene_list.lsk_bm.'+str(i)+'.tab',1,6)
...
print(data_ideas_matrix.shape)
### read rna-seq
data_rna = read_rna2matrix('/Users/gzx103/Documents/zhang_lab/projects/atacseq_vision/rna_seq_pred/rsem/lsk_bm.rsem.txt',1,4)
...
x_test0 = data_ideas_matrix[pos_test_id,:]
x_train0 = data_ideas_matrix[pos_train_id,:]
x_test = x_test0 - x_test0
x_train = x_train0 - x_train0

y_test0 = data_rna[pos_test_id,:]
y_train0 = data_rna[pos_train_id,:]
y_test = y_test0 - y_test0
y_train = y_train0 - y_train0

#cell_types = ['lsk_bm','cmp','gmp','mep','meg','cfuMK_ad','cfuE_ad','ery','gra_bm','mono_bm','g1e','er4']
...
for ct in cell_types:
	...
	x_test = np.concatenate(( x_test, (x_test0 - data_ideas_matrix[pos_test_id,:]) ), axis = 0)
	x_train = np.concatenate(( x_train, (x_train0 - data_ideas_matrix[pos_train_id,:]) ), axis = 0)
	#x_test = np.concatenate(( x_test, (x_test0) ), axis = 0)
	#x_train = np.concatenate(( x_train, (x_train0) ), axis = 0)

	y_test = np.concatenate(( y_test, (y_test0 - data_rna[pos_test_id,:]) ), axis = 0)
	y_train = np.concatenate(( y_train, (y_train0 - data_rna[pos_train_id,:]) ), axis = 0)
...
rnn_cell_1 = tf.contrib.rnn.GRUCell(rnn_cell_num)

outputs_1, states_1 = tf.contrib.rnn.static_rnn(rnn_cell_1, h_conv1_out_1, dtype=tf.float32)

W_atten1_1=weight_variable([rnn_cell_num, 1])
...
rnn_cell2 = tf.contrib.rnn.GRUCell(rnn_cell_num,reuse=True)

outputs_2, states_2 = tf.contrib.rnn.static_rnn(rnn_cell2, h_conv1_out_2, dtype=tf.float32)

W_atten1_2=weight_variable([rnn_cell_num, 1])
...
rnn_cell3 = tf.contrib.rnn.GRUCell(rnn_cell_num,reuse=True)

outputs_3, states_3 = tf.contrib.rnn.static_rnn(rnn_cell3, h_conv1_out_3, dtype=tf.float32, initial_state=(outputs_1[-1]+outputs_2[-1]) )

W_atten1_3=weight_variable([rnn_cell_num, 1])
...
for i in range(iter_num):
	...
	if i%100 == 0:
		...
		y_p = sess.run(y_conv, feed_dict={x_1: batch_xs1, x_2: batch_xs2, x_3: batch_xs3, keep_prob1: 1})
		y_obs=batch_ys1
	...
	if i%1000 == 0:
		y_p = sess.run(y_conv, feed_dict={x_1: x_test[:,0:sec_d,:,:], x_2: x_test[:,:sec_d_rev:-1,:,:], x_3: x_test[:,sec_d:sec_d_genebody,:,:], keep_prob1: 1})
		y_obs=y_test[:,:]
		y_mean=np.mean(y_obs)
		print(y_mean)
		...
		#rho, pval = spearmanr(y_p,y_obs)

		if 1==1:#accuracy_r2_test<=r2_test and accuracy_r2_train<=r2_train:
			saver = tf.train.Saver()
			save_path = saver.save(sess, "trained_rnn_3_noatac_dif.ckpt")

...
### read atac_seq pk
data_atac = read_tab2matrix('/Volumes/MAC_Data/data/labs/zhang_lab/vision_project/rna_seq_pred/matrix_for_rnn_atacpk/atac_21k_binary_split.gmp.tab',1,6)
### read ideas labels
...
h_conv1_out = tf.split(x_reshape,axis=0, num_or_size_splits=sec_d)
...
#tf.split(x_reshape, num_or_size_splits=sec_d, axis=1)
#print(h_conv1_out)
#rnn_cell2 = tf.contrib.rnn.LSTMCell(rnn_cell_num, forget_bias=0,use_peepholes=False,reuse=True)
...
#tf.split(x_reshape, num_or_size_splits=sec_d, axis=1)
#print(h_conv1_out)
#rnn_cell3 = tf.contrib.rnn.LSTMCell(rnn_cell_num, forget_bias=0,use_peepholes=False,reuse=True)
...
y_conv = (tf.matmul(lstm_tail, W_atten1)) + (tf.matmul(lstm_tail_2, W_atten1_2)) + (tf.matmul(lstm_tail_3, W_atten1_3))
...
index_array_p=np.arange(y_train.shape[0])
np.random.shuffle(index_array_p)
k=0
for i in range(iter_num):
	if (k+1)*batch_size > y_train.shape[0]:
		k=0
		index_array_p=np.arange(y_train.shape[0])
		np.random.shuffle(index_array_p)
	...
	batch_xs1_2=x_train[batch_id,:sec_d_rev:-1,:,:]
	batch_xs1_3=x_train[batch_id,sec_d:sec_d_genebody,:,:][:,::-1,:,:]

	batch_ys1=y_train[batch_id,:]
	k=k+1

	sess.run(train_step, feed_dict={x: batch_xs1, x_2: batch_xs1_2, x_3: batch_xs1_3, y_: batch_ys1, keep_prob1: 0.5})
	...
	if i%100 == 0:
		...
		y_p = sess.run(y_conv, feed_dict={x: batch_xs1, x_2: batch_xs1_2, x_3: batch_xs1_3, keep_prob1: 1})
		y_obs=batch_ys1
	...
	if i%1000 == 0:
		y_p = sess.run(y_conv, feed_dict={x: x_test[:,0:sec_d,:,:], x_2: x_test[:,:sec_d_rev:-1,:,:], x_3: x_test[:,sec_d:sec_d_genebody,:,:][:,::-1,:,:], keep_prob1: 1})
		y_obs=y_test[:,:]
		y_mean=np.mean(y_obs)
		print(y_mean)
		...
		if 1==1:#accuracy_r2_test<=r2_test and accuracy_r2_train<=r2_train:
			saver = tf.train.Saver()
			save_path = saver.save(sess, "trained_rnn1.ckpt")

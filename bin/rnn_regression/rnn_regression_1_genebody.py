...
for i in range(iter_num):
	...
	batch_xs1=x_train[batch_id,sec_d:sec_d_genebody,:,:]
	...
	if i%1000 == 0:
		y_p = sess.run(y_conv, feed_dict={x: x_test[:,sec_d:sec_d_genebody,:,:], keep_prob1: 1})
		y_obs=y_test[:,:]
		y_mean=np.mean(y_obs)
		print(y_mean)
		...
		if 1==1:#accuracy_r2_test<=r2_test and accuracy_r2_train<=r2_train:
			saver = tf.train.Saver()
			save_path = saver.save(sess, "trained_rnn_1_genebody.ckpt")

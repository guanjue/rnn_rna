...
### read ideas labels
data_ideas_tmp = read_tab2matrix('/Volumes/MAC_Data/data/labs/zhang_lab/vision_project/rna_seq_pred/matrix_for_rnn/matrix_for_rnn/gene_list.gmp.0.tab',1,6)
...
data_ideas_matrix = data_ideas_tmp.reshape([data_ideas_tmp.shape[0],data_ideas_tmp.shape[1],1,1])
for i in range(1,17):
	print(i)
...
for ct in cell_types:
	...
	data_atac = read_tab2matrix('/Volumes/MAC_Data/data/labs/zhang_lab/vision_project/rna_seq_pred/matrix_for_rnn_atacpk/atac_21k_binary_split.tab',1,6)
	### read ideas labels
	data_ideas_tmp = read_tab2matrix('/Volumes/MAC_Data/data/labs/zhang_lab/vision_project/rna_seq_pred/matrix_for_rnn/matrix_for_rnn/gene_list.'+ct+'.0.tab',1,6)
	#data_ideas_matrix = data_ideas_matrix * data_atac
	data_ideas_tmp = np.concatenate((data_ideas_tmp[:,0:sec_d-exp_win], data_ideas_tmp[:,sec_d_genebody+exp_win:all_len]), axis=1)
	data_ideas_matrix = data_ideas_tmp.reshape([data_ideas_tmp.shape[0],data_ideas_tmp.shape[1],1,1])
	for i in range(1,17):
...
x = tf.placeholder(tf.float32, shape=[None, up_down-exp_win*2, thr_d, for_d])
...
h_conv1_out = tf.split(x_reshape,axis=0, num_or_size_splits=up_down-exp_win*2)
...
for i in range(iter_num):
	...
	if i%1000 == 0:
		...
		if 1==1:#accuracy_r2_test<=r2_test and accuracy_r2_train<=r2_train:
			saver = tf.train.Saver()
			save_path = saver.save(sess, "trained_rnn_1_nogenebody_noatac.ckpt")
